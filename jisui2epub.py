...
# 和文に挟まれた半角ハイフン「-」の全角化。縦組み本文の中の半角ハイフンは
# ダッシュ「――」か長音「ー」をOCRが半角で返したもので、全角の連なりに半角が
# 1字だけ混ざるため読んでいて非常に目立つ（NDLOCR再OCRで顕著。どこよりも遠い
# 場所にいる君へ130件・赤毛のアン441件）。正解テキスト30本を調べた範囲では
# 正当な半角ハイフンが和文に挟まれる例は皆無で（手作業校正版に2件あるが
# どちらもそれ自体がOCR誤り）、ISBN・電話番号・型番・URLのハイフンは必ず
# 英数字に隣接する。そこで「両隣が和文」のものだけを全角ダッシュに置換する。
#   - 全角の判定に全角英数（Ａ-Ｚ・０-９）は含めない。ユーザーが半角ハイフンを
#     正当と感じる文脈（品番・年号の範囲）がこちら側にあるため
#   - 置換先を「――」でなく1字の「―」にするのは字数を変えないため。GOAL照合
#     628件の内訳はダッシュ499・長音48・その他81で、長音を「――」にすると
#     「ちーちゃん」が「ち――ちゃん」と1字伸びる（遠まわりする雛で実測37件）。
#     縦組みでは「―」と「ー」はほぼ同じ縦棒に見えるので、1字に留めておけば
#     どちらの誤りでも見た目が壊れない。**書籍単位でダッシュか長音かを
#     見分ける方法は見つからなかった**（「長音になりえない位置（前が約物・
#     漢字）の比率」は赤毛のアン0.12＝全件ダッシュ 対 遠まわりする雛0.10＝
#     全件長音 で重なる）
#   - OCRは同じ「―――」を「--」とも「---」とも返す（どこよりもp12とp13）ので
#     連続数はそのまま写す
#   - 直前の全角空白は許す（段落頭のダッシュ「　――どうして」を拾うため）。
#     直後は行末も許す（「　好きなの――」型の余韻のダッシュ。行末を許さないと
#     ［＃「…」は中見出し］の見出し名だけが変換されて本文行と食い違う）。
#     **行頭は許さない**＝横書き実用書の箇条書き「-項目」を壊すため
#   - 両隣が数字（漢数字含む）なら変換しない。奥付の郵便番号・電話番号・住所
#     （一〇一-〇〇四六、〇三-三二五二-三一一一）はハイフンが正しい
_JW_PUNCT = '、。・：；？！「」『』（）〔〕【】〈〉《》〜…‥―－'
_HW_DASH_RE = re.compile(
    rf'(?<=[{_JW_CHARS}{_JW_PUNCT}　])(-+)(?=[{_JW_CHARS}{_JW_PUNCT}]|\n|\Z)')
_NUMERAL_RE = re.compile(f'[{_DIGIT_CHARS}]')
...
def _fix_halfwidth_dash(text, stats):
    """和文に挟まれた半角ハイフンを全角ダッシュ「―」にした文字列を返す。"""
    def _repl(m):
        prev = text[m.start(1) - 1]
        nxt = text[m.end(1)] if m.end(1) < len(text) else ""
        if _NUMERAL_RE.match(prev) and (not nxt or _NUMERAL_RE.match(nxt)):
            return m.group(1)          # 郵便番号・電話番号・住所の区切り
        stats['全角ダッシュ―'] += 1
        return '―' * len(m.group(1))

    return _HW_DASH_RE.sub(_repl, text)
...
def normalize_ocr_text(text):
    ...
    text = _fix_halfwidth_dash(text, stats)

...
設計の要点（詳細は CLAUDE.md 参照）:
  ...
  - ルビは「縦書きの列幅」で判定する（列幅は本文と同じで高さだけ小さい
    句読点・小書き仮名を誤ってルビ扱いしないため）。ルビも本文の半分の
    フォントサイズで書き戻すことで、jisui2epub.py 本体の attach_rubies
    （フォントサイズ比0.68倍でルビを親文字に紐付け）がそのまま機能する
...
def insert_invisible_text(
    ...
    gapfilled = (
        _gapfill_missing_rubies(symbols, old_rubies, ruby_threshold, target_ruby_fontsize)
        if old_rubies
        else []
    )
    ...
    for text, rect in symbols:
        ...
        fontsize = target_ruby_fontsize if is_ruby else target_body_fontsize
        # 原点はベースライン左下相当。FONT_HEIGHT_RATIO分の割引は行わない
        # （下記コメント参照: 旧OCRページと混在する運用では危険なため撤回した）
        page.insert_text(
